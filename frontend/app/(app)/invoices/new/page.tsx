...
import type { Customer, Product, InvoiceLineItem } from '@/lib/types';
...
export default function NewInvoicePage() {
  ...
  const [lines, setLines] = useState<InvoiceLineItem[]>([
    { product_name: '', hsn_sac_code: '', unit: 'Nos', quantity: 1, rate: 0, discount: 0, gst_percent: 18 },
  ]);
  ...
  const save = async () => {
    const data = await api<{ invoice: { id: string } }>('/invoices', {
      method: 'POST',
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Invoice number will be assigned automatically (e.g. AETX-23-06-2026-01)"
      />
      <Paper sx={{ p: 2.5, mb: 2, border: '1px solid', borderColor: 'divider' }}>
        ...
        <TextField label="Due Date" type="date" slotProps={{ inputLabel: { shrink: true } }} value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
      </Paper>
      ...
      <Box sx={{ mt: 2.5, display: 'flex', gap: 2 }}>
        ...
        <Button variant="contained" startIcon={<SaveIcon />} onClick={save} disabled={!customerId || lines.length === 0}>Save Draft</Button>
      </Box>
    </>
  );
}
