...
const { getCustomer } = require('../customers/service');
const { getProduct, deductStock } = require('../products/service');
...
async function buildLineItemsFromInput(lines, taxType) {
  ...
  for (const line of lines) {
    let product = null;
    if (line.product_id) {
      product = await getProduct(line.product_id);
    }
    inputLines.push({
  ...
  try {
    ...
    for (const line of totals.lines) {
      await client.query(
        ...
        [
          invoice.id, line.product_id || null, line.product_name,
          line.hsn_sac_code, line.unit, line.quantity, line.rate, line.discount,
          line.gst_percent, line.taxable_value,
          line.cgst_rate, line.cgst_amount, line.sgst_rate, line.sgst_amount,
          line.igst_rate, line.igst_amount, line.line_total, sortOrder++,
        ]
      );
    }
...
async function updateInvoice(id, data, userRole) {
  ...
  try {
    ...
    if (data.line_items) {
      const customer = await getCustomer(data.customer_id || existing.customer_id);
      const org = await getOrgSettings();
      ...
      for (const line of totals.lines) {
        await client.query(
          ...
          [
            id, line.product_id || null, line.product_name,
            line.hsn_sac_code, line.unit, line.quantity, line.rate, line.discount,
            line.gst_percent, line.taxable_value,
            line.cgst_rate, line.cgst_amount, line.sgst_rate, line.sgst_amount,
            line.igst_rate, line.igst_amount, line.line_total, sortOrder++,
